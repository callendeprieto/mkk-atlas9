...
c        end subroutine
c      end interface
      OPEN(UNIT=17,file='fort.17',TYPE='OLD',READONLY,SHARED)
   10 CALL READIN(20)
      IFOP(14)=0
      IFOP(15)=0
      IFOP(16)=0
...
C     WAVENUMBERS MUST BE MULTIPLIED BY 1.E25
 ...
 1111 WRITE(6,15)IN,FRQEDG(IN),WLEDGE(IN),CMEDGE(IN)
   ...
   17 CONTINUE
      ...
      NUMNU=NUMNU+1
      FREQSET(NUMNU)=2.99792458E17/(ABS(WLEDGE(I))+ABS(WLEDGE(I+1)))*2.
      NUMNU=NUMNU+1
...
C
  ...
  260 XNFPEL(ION,NELEM)=XNFP(J,ION,NELEM)
   ...
   21 DOPPLE(5,60+NELEM)=DOPPLE(1,NELEM)
      ...
      DO 265 NELEM=40,99
  265 DOPPLE(6,NELEM)=SQRT(2.*TK(J)/MOMASS(NELEM-39)/1.660E-24+
     1VTURB(J)**2)/2.99792458E10
  270 CONTINUE
C
      IT=ITEMP
