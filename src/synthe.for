...
c     implicit REAL (KIND=4) (A-H,O-Z)
C********
...
c
     ...
     &,ACCESS='APPEND')
      OPEN(UNIT=13,file='fort.13',STATUS='NEW',FORM='UNFORMATTED')
 ...
 3440 WRITE(13)LINDAT8,LINDAT4
      ...
      write(6,666)WL,GFLOG,XJ,E,XJP,EP,CODE,LABEL,LABELP,
     1WL,NELION,GR,GS,GW,REF,NBLO,NBUP,ISO1,X1,ISO2,X2,OTHER1,OTHER2,
     2DWL,DGFLOG,DGAMMAR,DGAMMAS,DGAMMAW,WL
  666 FORMAT(F10.4,F7.3,F5.1,F12.3,F5.1,F12.3,F9.2,A8,A2,A8,A2/
 ...
 3442 CLOSE(UNIT=20,DISP='DELETE')
      ...
      OPEN(UNIT=14,file='fort.14',TYPE='NEW',FORM='UNFORMATTED'
     &,RECORDTYPE='FIXED',
     1ACCESS='DIRECT',
cBLOCKSIZE=LENREC*4,
     &RECORDSIZE=LENREC*4)
...
C     ADD LINES TO BUFFER
      ...
      QCONGF=CONGF
c
c qui per le molecole calcola un kapp0 troppo piccolo
c
...
C     TRANSPOSE
  ...
  899 CONTINUE
     ...
     1               IFTURB
      ...
      REAL*8 WAVE,WCON,EMERGE,WMERGE,WSHIFT,CONTX(25,17),WTAIL
...
c     END
      subroutine begtime
c      interface
c         function itime(time_array)
c         integer(4) time_array(3)
c         end function itime
c      end interface
       integer time(3)
          idum = itime (time)
          write(6,6)time
...
C   7 NLINE=ILINE-1
    ...
    3 FORMAT(F5.2,F10.4,A10,F5.1,F10.0,4F10.6)
      DO 5 I=1,4
    5 READ(GRIEM0200(ILINE*5-4+I),4)TTAB(I,iline),WIDTH(I,iline),
...
c     CON=0.564189583547756E0/(1.0+F1+F2)/DOPWL
      ...
      RETURN
      END
      FUNCTION DIMITRI(LINE,J,WAVEsyn,WL)
      PARAMETER (kw=99)
      DIMENSION WS(4),DS(4),ALFS(4),FORB1(4),FORB2(4),TS(4)
      COMMON /STATE/P(kw),XNE(kw),XNATOM(kw),RHO(kw),PTOTAL(kw)
      COMMON /TEMP/T(kw),TKEV(kw),TK(kw),HKT(kw),TLOG(kw),HCKT(kw),ITEMP
     ...
     2             SHLINE(kw),XNFPH(kw,2),XNFH(kw)
       REAL*8 WL,wavesyn
      DIMENSION WIDTH(6,999),SHIFT(6,999),WIDTHP(6,999),SHIFTP(6,999),
     1 WIDTHHE(6,999),SHIFTHE(6,999)
      dimension CODE(999),XNELOG(999),TTAB(6,999),wave(999),gam(999)
      DIMENSION XNETAB(7)
      CHARACTER*10 TRANS(6,999)
      DATA IREAD/0/
	wl4=sngl(wl)
...
c	shift=d
      DO 5 ILINE=1,69
	read(20,134)n,wave(iline),gam(iline)
134	format(i5,f10.1,f10.3)
	k=1
	read(20,133)xnetab(k)
c	type*,k,xnetab(k)
133	FORMAT(10X,E7.1)
      READ(20,3)TRANS(i,iline),(TTAB(I,iline),WIDTH(I,iline),
     1 SHIFT(I,iline),WIDTHP(I,iline),SHIFTP(I,iline),
     2 WIDTHHE(I,iline), SHIFTHE(I,iline),i=1,6)
    3 FORMAT(5x,A10,F8.0,6(1X,E10.2)/15x,f8.0,6(1x,e10.2)/15x,
	1 f8.0,6(1x,e10.2)/15x,f8.0,6(1x,e10.2)/15x,f8.0,
	2 6(1x,e10.2)/15x,f8.0,6(1x,e10.2))
	xnelog(iline)=alog10(xnetab(k))
	do 55 it=1,6
	IF(width(it,iline).ne.0.)
	1 shift(it,iline)=shift(it,iline)/width(it,iline)
	if(widthp(it,iline).ne.0.)
	1 shiftp(it,iline)=shiftp(it,iline)/widthp(it,iline)
	if(widthhe(it,iline).ne.0.)
	1 shifthe(it,iline)=shifthe(it,iline)/widthhe(it,iline)
55	continue
    5 continue
      IREAD=1
   10 continue
	do 500 i=1,69
	wave1=0.1*wave(i)-0.1
	wave2=0.1*wave(i)+0.1
        if(wl4.gt.wave1.and.wl4.lt.wave2)il=i
500	continue
      dimitri=0.
      E=XNE(J)
      TEMP=T(J)
...
C     TOTAL SHIFT IN ANGSTROM, then in nm
      ...
      dtot=dtot*0.1
C     VOIGT PARAMETERS
      FREQ=2.99792458E17/WL4
      FREQ4PI=FREQ*12.5664
      GAMMAR=2.223E13/WL4**2
	if(gam(il).gt.0.)gammar=10.**gam(il)
      GAMMAS=WTOT*FREQ4PI/WL4
      GAMRF=GAMMAR/FREQ4PI
      GAMSF=GAMMAS/FREQ4PI 
      A=(GAMRF+GAMSF)/DOPPLE(7)
C     NORMALIZATION CONSTANT
c     CON=0.564189583547756E0/(1.0+F1+F2)/(DOPPLE(7)*WL4)
      con=1.
      WWD=WAVEsyn4-WL4-DTOT
      DOPP=DOPPLE(7)*WL4
      dimitri=VOIGT(ABS(WWD)/DOPP,A)
      RETURN 
      END
