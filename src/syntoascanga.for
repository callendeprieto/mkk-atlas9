...
c
	wave=wave*10
	write(2,2301)wave,QMU(1),QMU(NMU1),resid
2301	format(f11.4,2E20.8,F13.6)
c     APLOT(IPLOT)=(1H )
   63 CONTINUE                                                                  
c   68 CONTINUE                                                                  
   70 CONTINUE                                                                  
...
2244	FORMAT(1X,'NLINES= ',I10)
      ...
      READ(1)LINDAT
      resid=center/concen
      WRITE(3,8)WL,GFLOG,XJ,E,XJP,EP,CODE,REF,resid
    8 FORMAT(F10.4,F7.3,F5.1,F12.3,F5.1,F12.3,F9.2,2x,A4,1x,f8.4)
    9 CONTINUE
      CALL EXIT
      END
